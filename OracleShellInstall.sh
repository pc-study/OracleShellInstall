	...
	if ((node_num == 1)); then
		if check_file "$grid_soft_name"; then
			...
			"21")
				...
				;;
			"26")
				check_md5sum "$grid_soft_name" "d1161f1297593e5a681400f0dc124b9e"
				;;
			esac
		else
		...
		if ((db_version == 11)); then
			...
			if check_file "$db_soft_name"; then
				...
				"21")
					...
					;;
				"26")
					check_md5sum "$db_soft_name" "91f4b49dfd586df2bc904ab61aedeff7"
					;;
				esac
			else
...
alias dblog='tail -200f \$ORACLE_BASE/diag/rdbms/*/\$ORACLE_SID/trace/alert_\$ORACLE_SID.log'"
		...
		# 11gR2 在 RHEL10 上安装时需要设置 LD_LIBRARY_PATH
		if [[ $oracle_install_mode =~ ^(rac|standalone)$ ]]; then
...
\`\`\`bash
$(cat /proc/$$/cmdline 2>/dev/null | tr '\0' ' ' || echo "sh OracleShellInstall.sh [参数]")
\`\`\`
...
\`\`\`
-- 请粘贴以下命令的输出:
-- su - oracle -c "sqlplus / as sysdba <<< 'select instance_name, status from v\\\$instance;'"
\`\`\`
...
*投稿：https://www.oracleshellinstall.com/contribute.html*
REPORT_EOF

	color_printf green "安装报告已生成：$report_file"
	color_printf green "您可以编辑该文件，记录安装体验并投稿分享。"
}
#==============================================================#
#                    安装完成分享引导                            #
#==============================================================#
function print_share_guide() {
	...
	printf "${cyan}  ║${yellow}  1. 安装报告已保存在 /soft 目录下                            ${cyan}║${res}\n"
	printf "${cyan}  ║${yellow}  2. 编辑报告，补充截图和使用感受                              ${cyan}║${res}\n"
